...
export default function ProductDetail({ slug ,product}: { slug: string; product: Product }) {
    ...
    const fetchProductsMore = async (cursor: string | null = null, append = false) => {
        try {
            ...
            if (!res.ok) throw new Error('Gagal mengambil produk lain')
            const data = await res.json()

            setNextCursor(data.next || null)
            setProductsMore(prev =>
    ...
    if (!products) return <ClientPreloader />
    return (
        <>
            {products && <div className="font-sans bg-yellow-50 text-red-900 overflow-x-hidden">
                <ClientNavbar />
